...
Backend-neutral terminal-multiplexer seam (mirrors the `providers/` seam). Callers import the module-level `multiplexer` proxy and type against `multiplexer.base.Multiplexer`; they must not import a concrete backend (`multiplexer.tmux`/`multiplexer.herdr`/`multiplexer.agterm`). Enforced by the F1 boundary audit (`tests/ccgram/test_multiplexer_boundary.py`). Backend selected by `CCGRAM_MULTIPLEXER` (default `tmux`), wired in `bootstrap.py` from `config.multiplexer_name`.

- `base.py` — core, pure: `Multiplexer` Protocol, `MultiplexerCapabilities` dataclass (`name` + twelve capability flags: `ids_stable_across_restart`, `exposes_pane_tty`, `native_agent_status`, `read_max_lines`, `self_identify_env`, `supports_event_stream`, `native_worktrees`, `supports_display_name_rebind`, `recovers_stale_ids_by_name`, `supports_workspace_selection`, `native_topic_targets`, `supports_shell_prompt_markers`), and neutral value types `WindowRef`, `PaneInfo`, `CaptureResult`, `ForegroundInfo`, `PaneDims`, `AgentStatus`, `MuxEvent`. No backend imports, no I/O library (F3 core-purity audit). `watch_events(window_ids) -> AsyncGenerator[MuxEvent, None]` streams push events (`kind` ∈ `{"agent_status","window_died"}`) on `supports_event_stream` backends (herdr); tmux returns an empty async generator. Consumed by `event_stream_monitor.EventStreamMonitor`, gated on `supports_event_stream`. `agent_status(window_id) -> AgentStatus | None` exposes native agent run-state on `native_agent_status` backends (herdr and agterm); tmux returns None. `split_window(window_id) -> str | None` adds a sibling pane (herdr `pane split`, tmux `window.split()`) — the multi-pane "agent team" shape, driven by the `/split` command. `create_worktree_window(repo_path, worktree_path, branch, …) -> (ok, msg, name, window_id)` creates a git worktree _and_ the window running in it in one step on `native_worktrees` backends (herdr); tmux returns a not-supported failure tuple. `WindowRef.topic_eligible` is the backend-owned verdict on whether unattended discovery may adopt a live window. It travels on complete reconciliation listings so liveness and adoption remain separate; `WindowRef.alias_window_ids` remains the migration channel for superseded identities.
- `tmux.py` — adapter: `TmuxManager` (tmux backend) satisfying `Multiplexer`, returns the neutral value types, exposes tmux `capabilities` (`ids_stable_across_restart=True`, `exposes_pane_tty=True`, `native_agent_status=False`, `read_max_lines=None`, `self_identify_env="TMUX_PANE"`, `supports_event_stream=False`, `native_worktrees=False`, `supports_display_name_rebind=True`, `recovers_stale_ids_by_name=False`, `supports_workspace_selection=False`, `native_topic_targets=False`, `supports_shell_prompt_markers=True`). `create_worktree_window` returns a not-supported failure tuple (callers gate on `native_worktrees`). Owns the single `tmux_manager` singleton. `foreground(window_id)` is the sole `pane_tty` + `ps -t <tty>` site in the codebase (the private detail behind the seam's foreground source); shell tool-detection and provider auto-detection read it through the `multiplexer.foreground()` proxy, never a tty.
- `herdr.py` — adapter: `HerdrManager` (herdr backend) satisfying `Multiplexer`, an anti-corruption layer over the `herdr` CLI/socket. All herdr JSON shapes and `wN:tN`/`wN:pN` ids stay private; methods return the neutral value types. **Guarded session identity:** `window_id` is an opaque `herdr-session-v1-…` target derived from Herdr's complete `agent_session` composite — one ccgram topic = one reported agent session; bare shells and unsupported sessions do not surface; known Claude, Codex and Gemini hook-capable sessionless records use a terminal fallback identity; Pi is deliberately excluded. I/O-free constructor with an injectable command runner (unit-tested with JSON fixtures); `HERDR_PROTOCOL_VERSION` is the maximum of the static supported set (14–20). `ensure_session` accepts supported versions silently and warns but continues for versions outside that set or a false CLI compatibility flag, leaving transport and schema failures to individual commands. `list_windows` builds one `WindowRef` per complete agent-session record: it resolves workspace/tab labels, includes the pane locator for display, and stamps `format_agent_topic_prefix(workspace_label, tab_label, pane, provider=agent)` → `"<Provider> ▸ <workspace> ▸ <tab> ▸ <pane>"` into `WindowRef.window_name`; skips workspace/tab labels matching `^__.*__$` so ccgram never auto-adopts itself. `create_window` returns a guarded opaque agent-session target as `window_id`. Agent actions guard the target against a fresh `agent.list` snapshot and use that record's current pane locator; sessionless, missing, malformed, and duplicate targets fail closed. `kill_window` closes the resolved pane; `rename_window` renames the resolved tab. herdr `capabilities` (`ids_stable_across_restart=False`, `exposes_pane_tty=False`, `native_agent_status=True`, `read_max_lines=1000`, `self_identify_env="HERDR_PANE_ID"`, `supports_event_stream=True`, `native_worktrees=True`, `supports_display_name_rebind=False`, `recovers_stale_ids_by_name=False`, `supports_workspace_selection=True`, `native_topic_targets=True`, `supports_shell_prompt_markers=True`). `create_worktree_window` delegates to one `herdr worktree create --branch <ccg/…> --path <path> --no-focus --json` (makes the checkout + a workspace+tab+pane grouped under the parent repo) then `pane run`s the launch command; the `/new` worktree step routes through it (gated on `native_worktrees`), skipping ccgram's `git worktree add` and the workspace picker. `foreground(window_id)` maps `pane process-info.foreground_processes[]` → `ForegroundInfo` (pid/argv/cwd; `pgid` from `foreground_process_group_id`) — no tty, since `exposes_pane_tty=False`. `agent_status(window_id)` reads the active pane's `pane.agent_status` (`working`/`idle`/`done`/`blocked`/`unknown`) → `AgentStatus`; consumed by `observe._native_agent_status` (push cache first, this subprocess as cold-cache fallback) to gap-fill the status line for non-Claude agents when terminal scraping yields nothing. `watch_events(window_ids)` opens a persistent `events.subscribe` connection (the only long-lived socket reader; socket I/O isolated in `herdr_events.py`, injectable as `stream_opener`) — global `tab.closed` plus target-specific `pane.exited`/`pane.closed` death events, and per-pane `pane.agent_status_changed`; reprimes status after the subscribe is live, reconnects with backoff on drop. herdr's `event` form is inconsistent (dot `pane.agent_status_changed` vs underscore `tab_closed`); `herdr_events.translate_event` matches both. Scrollback captures clamp to `read_max_lines` (1000) and set `CaptureResult.truncated` so a >1000-line command surfaces truncation instead of silently dropping output. Session-map key is `herdr:<opaque-target-id>`; `session_map_prefix()` used in cleanup/lifecycle/transcript_discovery (backend-neutral key surface). CB_PANE_DELIMITER `"|"` used for pane callback encoding (avoids collision with the colon in herdr ids). Workspace picker step in `/new` flow gated on `supports_workspace_selection`. Contract validated by `tests/ccgram/test_herdr_backend.py` (unit) + `tests/integration/test_herdr_contract.py` (`-m herdr`, live socket); shell-on-herdr by `tests/integration/test_shell_herdr.py` (`-m herdr`).
- `agterm.py` — adapter: `AgtermManager` (agterm backend) satisfying `Multiplexer`, an anti-corruption layer over the `agtermctl` CLI/socket. All agterm JSON shapes and `surface:<uuid>:<role>` locators stay private. **Session identity:** `window_id = the agterm session UUID` — one ccgram topic = one agterm session; agterm persists and restores that UUID in its per-window state file, so `ids_stable_across_restart=True` and no alias reconciliation is needed. I/O-free constructor with an injectable command runner (unit-tested with JSON fixtures). agterm `capabilities` (`ids_stable_across_restart=True`, `exposes_pane_tty=False`, `native_agent_status=True`, `read_max_lines=None`, `self_identify_env="AGTERM_SESSION_ID"`, `supports_event_stream=False`, `native_worktrees=False`, `supports_display_name_rebind=True`, `recovers_stale_ids_by_name=False`, `supports_workspace_selection=True`, `native_topic_targets=False`, `supports_shell_prompt_markers=False`). **Native agent status.** agterm reports status through `tree.status`; topic eligibility is carried by `WindowRef.topic_eligible`, so native status and topic adoption are independent concerns. Workspace selection is gated by `supports_workspace_selection`. **Explicit pane on every call:** `session text` with no `--pane` reads the visible/focused pane while `session type` with no `--pane` injects into the main pane, so every capture and injection names `left` (where the agent runs) instead of relying on either default. **No sibling handle:** agterm promotes the split survivor into the main pane when the primary exits, so the `right` role is not a stable identity — `split_window` returns None and `list_panes` returns `[]`, per the Protocol's rule for a backend that cannot expose a safe sibling locator. `send(..., literal=False)` translates tmux key names (`Escape`, `C-c`, `Up`, `M-Enter`) to pty bytes via a private table and passes them on **stdin**, never argv; agterm's `session type` writes every byte through verbatim apart from newlines, which become a Return keypress. `foreground(window_id)` maps `tree.foreground` → `ForegroundInfo` (argv and cwd only; pid/pgid are 0 and tty is empty, since `exposes_pane_tty=False`). `providers/process_detection.py` classifies agterm's foreground argv even when `fg.pgid == 0`; providers named directly still resolve through `WindowRef.pane_current_command`, while wrapped providers use the argv fallback without caching. `capture_*` ignore `with_ansi`: agterm returns UTF-8 grid text with no SGR, but spacing, row boundaries and box-drawing survive intact, so structural parsing is unaffected and only rendered screenshots lose styling (the renderer falls back to default styles). `create_window` passes `--no-select` so an automated creation never steals the selection from whoever is at the keyboard. `create_worktree_window` returns a not-supported failure tuple. Session-map key is `agterm:<session-uuid>`. **Discovery scope:** agterm has no per-application container the way tmux has its own session, so `list_windows` is filtered to the `CCGRAM_AGTERM_WORKSPACES` environment value (default `ccgram`, `*` for all) plus the self/underscore exclusions; without it every session the user has open would surface as a topic. `list_windows_for_reconciliation` is deliberately **unfiltered** — cleanup asks what exists, not what may be adopted, so a bound session moved to another workspace does not read as gone and get pruned. **Reads merge every open window:** `tree` is placement-scoped to the frontmost window (`controlTree` → `resolvePlacementStore` → `library.activeStore`), so `_tree()` enumerates `window list` and merges `tree --window <id>`, skipping closed windows and failing closed on any error; `--target` session commands resolve cross-window and need no widening. Agterm classifies a known bare shell from `foregroundShell` and `splitForegroundShell`; this does not prove the shell is at an idle prompt. A builtin or loop has the same shell name, so `supports_shell_prompt_markers=False` prevents prompt setup from typing syntax into a running command. `doctor` and `status` dispatch on backend name (an agterm socket check; a seam-based listing) instead of falling through to tmux. Contract validated by `tests/ccgram/test_agterm_backend.py`.
- `registry.py` — `get_multiplexer(name)` + singleton cache (mirrors `providers/registry.py`); backends (`tmux`, `herdr`, `agterm`) imported lazily inside their factory so the core stays I/O-free. `UnknownMultiplexerError` on unknown names.
...
- `herdr_events.py` — herdr push-event-stream plumbing (the only long-lived unix-socket reader). `open_socket_stream(socket_path, subscriptions)` async-generates pushed event dicts (yields a `SUBSCRIBED` sentinel after the ack so the caller reprimes once the subscription is live); `translate_event(obj, pane_to_window, tab_to_windows) -> tuple[MuxEvent, ...]` maps a herdr event dict → neutral events for watched targets; a `tab.closed` fans out to every target sharing that tab. Injected into `HerdrManager` as `stream_opener` for socket-free unit tests.
...
- `self_identify.py` — backend-neutral hook identity resolver (`resolve_self_identity(env)` → `SelfIdentity`). Picks the backend by which `self_identify_env` var is present, in order (`$TMUX_PANE` → tmux via injected `tmux_query`; `$HERDR_PANE_ID` → herdr; `$AGTERM_SESSION_ID` → agterm, no probe). agterm is checked **last**: every shell agterm spawns inherits `AGTERM_SESSION_ID`, including one running a nested tmux or herdr, so an earlier check would claim panes owned by the inner multiplexer. I/O-free: the tmux `display-message` probe is injected by `hook.py` (its `_resolve_window_id`), so the hook (separate process) keeps the subprocess and the resolver stays table-testable. The tmux branch is byte-identical to the previous direct path.
- `topic_mapping.py` — backend-neutral projection of multiplexer windows onto Telegram topics (consumer of the seam, not part of the `Multiplexer` contract). Two pure helpers. `is_agent_topic_window(window, caps)`: the discovery filter for whether a live window may be adopted; it returns the backend-stamped `WindowRef.topic_eligible`. Topic eligibility is independent of native agent status: Herdr and agterm stamp only agent windows as eligible, while tmux ordinary windows are eligible and excluded windows are marked ineligible in reconciliation. `format_agent_topic_prefix(workspace, tab, pane="", *, provider="")`: renders `"<Provider> ▸ <workspace> ▸ <tab> ▸ <pane>"` for Herdr agent topics while empty parts degrade gracefully; the Herdr adapter stamps it into `WindowRef.window_name`; the status emoji is prepended later by `topic_emoji`. Imported by `session_monitor._emit_unbound_window_events` (discovery) and `multiplexer.herdr` (labels). Lives outside the tmux backend so core + handlers can use it without crossing the F1 boundary.
...
- Startup re-resolution. Window IDs reset on tmux server restart. `resolve_stale_ids()` remaps missing IDs by display name only on `recovers_stale_ids_by_name` backends (tmux); stable agterm UUIDs are retained and Herdr's durable targets are not remapped. Old name-keyed `state.json` is auto-migrated.
...
- Multiplexer seam. Terminal-multiplexer access is abstracted behind the `Multiplexer` Protocol (`multiplexer/base.py`, core, I/O-free). tmux is the default backend (`multiplexer/tmux.py`); a `CCGRAM_MULTIPLEXER` switch (default `tmux`, also `herdr` and `agterm`) selects it, wired in `bootstrap.py` from `config.multiplexer_name`. Callers depend only on the module-level `multiplexer` proxy and `MultiplexerCapabilities` flags — never a concrete backend or `name == "<backend>"` conditional. Boundary enforced by `tests/ccgram/test_multiplexer_boundary.py` (F1, concrete-backend imports forbidden outside `multiplexer/**`, `bootstrap.py`, `main.py`), contract by `tests/ccgram/test_multiplexer_contract.py` (F2, parametrized per backend), core purity by `tests/integration/test_import_no_cycles.py` (F3, `base` imports no backend/I/O). Mirrors the `AgentProvider` seam and the `window_store`/`thread_router` proxy pattern.
