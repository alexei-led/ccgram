...
- `topic_mapping.py` — backend-neutral projection of multiplexer windows onto Telegram topics (consumer of the seam, not part of the `Multiplexer` contract). Pure `is_agent_topic_window(window, caps)`: the capability-gated discovery filter for "topic = pane = agent". On `native_agent_status` backends (herdr) only agent panes (non-empty `WindowRef.pane_current_command`) surface as topics — a bare shell pane does not; on tmux every window is eligible (unchanged). Imported by `session_monitor._emit_unbound_window_events`. Lives outside the tmux backend so core + handlers can use it without crossing the F1 boundary.
