...
- `herdr.py` — adapter: `HerdrManager` (herdr backend) satisfying `Multiplexer`, an anti-corruption layer over the `herdr` CLI/socket. All herdr JSON shapes and `wN:pN` ids stay private; methods return the neutral value types. Opaque `pane_id` _is_ `window_id` (thin identity). I/O-free constructor with an injectable command runner (unit-tested with JSON fixtures); pins `HERDR_PROTOCOL_VERSION` from `herdr status` and refuses on mismatch (`HerdrProtocolError`). `list_windows` stamps the adaptive topic label (`format_agent_topic_prefix` over `workspace list`/`tab list` labels + per-tab pane counts) into `WindowRef.window_name`, the single source feeding topic discovery and display-name re-sync (so a workspace/tab rename re-labels the bound topic on the next poll without rebinding); `create_window` resolves cwd→workspace (reuse the workspace matching the cwd, create only if absent) before `tab create --workspace`. herdr `capabilities` (`ids_stable_across_restart=False`, `exposes_pane_tty=False`, `native_agent_status=True`, `read_max_lines=1000`, `self_identify_env="HERDR_PANE_ID"`, `supports_event_stream=True`). `foreground(window_id)` maps `pane process-info.foreground_processes[]` → `ForegroundInfo` (pid/argv/cwd; `pgid` from `foreground_process_group_id`) — no tty, since `exposes_pane_tty=False`. Scrollback captures clamp to `read_max_lines` (1000) and set `CaptureResult.truncated` so a >1000-line command surfaces truncation instead of silently dropping output. Contract validated by `tests/ccgram/test_herdr_backend.py` (unit) + `tests/integration/test_herdr_contract.py` (`-m herdr`, live socket); shell-on-herdr by `tests/integration/test_shell_herdr.py` (`-m herdr`).
...
- `topic_mapping.py` — backend-neutral projection of multiplexer windows onto Telegram topics (consumer of the seam, not part of the `Multiplexer` contract). Two pure helpers. `is_agent_topic_window(window, caps)`: the capability-gated discovery filter for "topic = pane = agent". On `native_agent_status` backends (herdr) only agent panes (non-empty `WindowRef.pane_current_command`) surface as topics — a bare shell pane does not; on tmux every window is eligible (unchanged). `format_agent_topic_prefix(workspace, agent, tab, *, split)`: renders the herdr adaptive topic label `"<workspace> ▸ <agent>"` (+ `"/<tab>"` only when the tab is split), which the herdr adapter stamps into `WindowRef.window_name`; the status emoji is prepended later by `topic_emoji`. Imported by `session_monitor._emit_unbound_window_events` (discovery) and `multiplexer.herdr` (labels). Lives outside the tmux backend so core + handlers can use it without crossing the F1 boundary.
