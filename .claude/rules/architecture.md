...
- `base.py` — core, pure: `Multiplexer` Protocol, `MultiplexerCapabilities` dataclass (`name` + six capability flags: `ids_stable_across_restart`, `exposes_pane_tty`, `native_agent_status`, `read_max_lines`, `self_identify_env`, `supports_event_stream`), and neutral value types `WindowRef`, `PaneInfo`, `CaptureResult`, `ForegroundInfo`, `PaneDims`, `AgentStatus`. No backend imports, no I/O library (F3 core-purity audit). `agent_status(window_id) -> AgentStatus | None` exposes native agent run-state on `native_agent_status` backends (herdr); tmux returns None.
