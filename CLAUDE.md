...
State files (in config dir): `state.json` (thread bindings, window states, display names, read offsets), `session_map.json` (hook-generated), `events.jsonl` (hook events), `monitor_state.json` (byte offsets), `mailbox/` (inter-agent inboxes).
...
Agents in tmux windows can discover each other, exchange messages, broadcast, and spawn new agents — with human oversight via Telegram.

CLI (`ccgram msg`):

```
list-peers [--json]                # active agent windows
find --provider X --team Y         # filter peers
send <to> <body> [--wait] [--notify]
inbox [--json] | read <id> | reply <id> <body>
broadcast <body> [--team X]
register --task "..." --team "..."  # declare for discovery
spawn --provider X --cwd ~/proj    # request new agent (needs approval)
sweep                              # clean expired
```

Design:

- File-based mailbox (`~/.ccgram/mailbox/`): per-window inbox dirs, timestamp-prefixed JSON messages, atomic writes.
- Qualified IDs: `session:@N` (e.g. `ccgram:@0`).
- Broker delivery: poll loop injects pending messages into idle agent windows via `send_keys`. Shell windows are inbox-only.
- Telegram visibility: silent notifications in both topics, grouped, edit-in-place for replies.
- Spawn approval: user approves via Telegram inline keyboard, auto-creates topic.
...
Config: `CCGRAM_MSG_AUTO_SPAWN` (`false`), `CCGRAM_MSG_MAX_WINDOWS` (`10`), `CCGRAM_MSG_WAIT_TIMEOUT` (`60s`), `CCGRAM_MSG_SPAWN_TIMEOUT` (`300s`), `CCGRAM_MSG_SPAWN_RATE` (`3/window/hr`), `CCGRAM_MSG_RATE_LIMIT` (`10/window/5m`).
