...
def register_terminal_backends() -> None:
    """Register optional terminal backends with the router.

    Tmux is auto-registered the first time ``get_router()`` is called;
    ...
    # transitively pulls asyncio Unix-socket primitives. Keep both
    # off the bootstrap cold path when cmux is disabled.
    from .terminal_backends.config import load_terminal_backend_config

    # Lazy: same reason — lifecycle imports CmuxSidecarClient.
    from .terminal_backends.lifecycle import register_cmux_backend_if_enabled
