...
graph TB
    ...
    SM2 --> MQ["messaging_pipeline/message_queue.py<br>delivery receipts"]
...
| Delivered transcript watermark                    | The transcript reader advances only an in-memory parsed offset. `message_queue` settles the receipt for every queued transcript task, and the monitor persists that session's watermark only after all receipts succeed or are intentionally dropped; failures remain replayable after restart.                                                                                                                                                                                                 |
