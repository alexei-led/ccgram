...
- **Skills** — `SKILL.md` under `~/.omp/agent/skills/<name>/`, `~/.agents/skills/<name>/`, `<project>/.omp/skills/<name>/`, or `<project>/.agents/skills/<name>/`. Loose `.md` files at the root of `~/.omp/agent/skills/` or `<project>/.omp/skills/` are also picked up. Oh My Pi registers a skill as `/skill:<name>`, so ccgram advertises and forwards that qualified name. Pi's skill commands are bare, so only Oh My Pi carries the prefix.
